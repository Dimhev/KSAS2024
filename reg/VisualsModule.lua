...
return function(visualsTab, library)
    ...
    local function createEspDrawing(plr)
        ...
        c.hpOutline.Thickness = 1
        c.hpOutline.Color = Color3.new(0, 0, 0)
        c.hpOutline.Filled = true
    ...
    local function registerObject(inst)
        ...
        if inst:IsA("ParticleEmitter") or inst:IsA("Smoke") or inst:IsA("Trail") or inst:IsA("Beam") or inst:IsA("Light") then
            ...
            if inst:IsA("ParticleEmitter") then 
                vfxData.Type = "Particle"
                vfxData.Rate = inst.Rate 
            elseif inst:IsA("Smoke") then 
                vfxData.Type = "Smoke"
                vfxData.Opacity = inst.Opacity 
            elseif inst:IsA("Trail") then 
                vfxData.Type = "Trail"
                vfxData.Lifetime = inst.Lifetime 
            elseif inst:IsA("Beam") then 
                vfxData.Type = "Beam"
                vfxData.Segments = inst.Segments 
            elseif inst:IsA("Light") then 
                vfxData.Type = "Light"
                vfxData.Shadows = inst.Shadows
                vfxData.Range = inst.Range 
            end
