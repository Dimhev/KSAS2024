...
local closeBtn = create("TextButton", { Size = UDim2.new(0, 40, 0, 40), Position = UDim2.new(1, -40, 0, 0), BackgroundTransparency = 1, Text = "X", TextColor3 = Color3.fromRGB(180, 180, 180), TextSize = 14, Font = fontBold, Parent = topBar })
...
function library:SetSyncMode(state)
    ...
    local function updateTheme()
        ...
        if isFirstUpdate then
            ...
            ts:Create(mainVal, tweenInfo, {Value = newMain}):Play()
            ts:Create(elementVal, tweenInfo, {Value = newElement}):Play()
            ts:Create(accentVal, tweenInfo, {Value = newAccent}):Play()
        end
    end
